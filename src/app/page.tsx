  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-5">
        ...
        {/* ── Two-tab output ── */}
        {hasReport && (
          <section ref={reportRef} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            {/* Tab bar */}
            <div className="flex items-center justify-between border-b border-slate-200 px-6 pt-4">
              <div className="flex gap-0">
                {(["report", "technical"] as const).map((tab) => (
                  <button
                    key={tab}
                  ...
                  >
                    {tab === "report" ? "Research Report" : "Technical"}
                  </button>
                ))}
              </div>
              <button
                onClick={downloadReport}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors mb-1"
              >
            ...
            <div className="p-6">
              {/* Report tab — structured markdown */}
              {activeTab === "report" && (
                <MarkdownReport text={session.finalAnswer} />
              )}

              {/* Technical tab — per-agent raw outputs */}
              {activeTab === "technical" && (
                <div className="space-y-4">
                  {AGENTS.map((agent) => {
                    ...
                    return (
                      <div key={agent.id} className="rounded-lg border border-slate-200 overflow-hidden">
                        <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 border-b border-slate-200">
                          <span className="w-2 h-2 rounded-full bg-emerald-500 flex-shrink-0" />
                          <span className="text-xs font-semibold text-slate-700">{agent.codename}</span>
                          <span className="text-xs text-slate-400 truncate">{agent.role}</span>
                          <span className="ml-auto text-xs text-slate-400">{output.content.length} chars</span>
                        </div>
                        <pre className="p-4 text-xs text-slate-600 font-mono whitespace-pre-wrap leading-relaxed overflow-x-auto max-h-64 overflow-y-auto">
                          {output.content}
                        </pre>
                      </div>
                    );
                  })}
                  {AGENTS.every((a) => !session.outputs[a.id]?.content) && (
                    <p className="text-sm text-slate-400 text-center py-6">No agent outputs captured.</p>
                  )}
                </div>
              )}
            </div>
