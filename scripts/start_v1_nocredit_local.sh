...
RETRIEVAL_BUDGET_SECONDS="40"
if [[ "$USE_SEARXNG" == "1" && "$GUARANTEE_INTERNET_RETRIEVAL" == "1" ]]; then
  RETRIEVAL_BUDGET_SECONDS="90"
fi
...
if [[ "$USE_SEARXNG" == "1" && "$GUARANTEE_INTERNET_RETRIEVAL" == "1" ]]; then
  export HEXAMIND_RETRIEVAL_TIMEOUT_SECONDS="$RETRIEVAL_BUDGET_SECONDS"
  export HEXAMIND_STAGE_TIMEOUT_RETRIEVAL_SECONDS="$RETRIEVAL_BUDGET_SECONDS"
else
  export HEXAMIND_RETRIEVAL_TIMEOUT_SECONDS="${HEXAMIND_RETRIEVAL_TIMEOUT_SECONDS:-$RETRIEVAL_BUDGET_SECONDS}"
  export HEXAMIND_STAGE_TIMEOUT_RETRIEVAL_SECONDS="${HEXAMIND_STAGE_TIMEOUT_RETRIEVAL_SECONDS:-$RETRIEVAL_BUDGET_SECONDS}"
fi
