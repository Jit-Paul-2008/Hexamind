...
- Graceful degraded mode that sti
...
- Provider health manager with circuit breaker and retry budget
- Timeout budget by stage (retrieval, per-agent generation, final synthesis)
- Graceful degraded mode that sti' through a five-agent adversarial pipeline. Primary synthesis: Taken together, the evidence on 'Autonomous research task. Evidence base includes 0 retrieved sources; confidence is low.
